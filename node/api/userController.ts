...
const saveFiles = (req, res) =>{ //too bad it works the same in every controller already
    sharp(req.file.path).resize({ width: 150 }).toFile(`${__dirname}/../uploads/avatars/small-${req.file.originalname}`, (err, resizeImage)=>{
      if(err){
        returnError(res, err);
      } 
    });
    sharp(req.file.path).resize({ width: 300 }).toFile(`${__dirname}/../uploads/avatars/medium-${req.file.originalname}`, (err, resizeImage)=>{
      if(err){
        returnError(res, err);
      }
    });

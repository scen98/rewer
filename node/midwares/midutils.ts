import { getConnection } from "../config/mysqlConnection";

export function validateInput(){
    
}

export async function withMysql(callback: (connection)=>Promise<any>, callbackerr: (err)=>any){
    const mySqlConnection = getConnection();
    try{
        await callback(mySqlConnection);
    } catch (error){
        callbackerr(error);
    } finally{
        mySqlConnection.end();
